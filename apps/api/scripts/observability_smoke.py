...
def smoke_request_correlation_chat_and_upload() -> None:
    ...
    try:
        ...
        with (
            ...
            patch(
                "app.api.upload.insert_document_and_chunks",
                return_value=("doc-smoke", 1),
            ),
            TestClient(app, raise_server_exceptions=False) as client,
        ):
            mock_extract_text.return_value = "hello world"
            mock_cur = (
                mock_get_conn.return_value.__enter__.return_value.cursor.return_value
            )
            mock_cur.__enter__.return_value = mock_cur
            mock_embeddings_provider.return_value.embed_documents.return_value = [
                [0.1] * 384
            ]
            ...
            _assert(
                ...
                f"chat status expected 200 got {chat_resp.status_code}",
            )
            _assert(
    ...
    try:
        with (
            ...
            _assert(
                ...
                f"metrics status {metrics_resp.status_code}",
            )
            body = metrics_resp.text

        _assert(
            "atlas_http_requests_total" in body, "missing atlas_http_requests_total"
        )
        _assert(
            re.search(
